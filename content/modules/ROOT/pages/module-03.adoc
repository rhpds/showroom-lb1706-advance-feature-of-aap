...
In your browser open {ansible_controller_web_ui_url}[window=_blank] and login as user `{ansible_controller_admin_username}` with the password `{ansible_controller_admin_password}`.
